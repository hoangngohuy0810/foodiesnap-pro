...
export default function App() {
  ...
  const generateImages = async () => {
    if (!foodImage) {
      setError("Vui lòng tải ảnh món ăn lên trước.");
      return;
    }
    ...
    try {
      ...
      // Ensure URL works in dev and prod
      const API_URL = (import.meta as any).env?.VITE_API_URL || '/api/generate';
      ...
      if (!response.ok) {
        throw new Error(data.error || "Lỗi server khi tạo ảnh.");
      }
  ...
  const handleBatchDownload = async () => {
    ...
    // We import dynamically so we don't need the top level import JSZip
    const { default: JSZip } = await import('jszip');
    const zip = new JSZip();
    
    try {
      await Promise.all(results.map(async (img, idx) => {
      ...
      console.error("Lỗi khi tải file zip", e);
      setError("Không thể tải xuống tất cả ảnh cùng lúc do lỗi mạng.");
    }
  };
    ...
    <div className="min-h-screen pb-20">
      ...
      <header className="px-6 py-8 flex items-center justify-between max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">FoodieSnap <span className="text-brand-orange italic">Pro</span></h1>
            <p className="text-xs text-gray-500 uppercase tracking-widest font-mono">Studio nhiếp ảnh món ăn AI</p>
          </div>
        </div>
          ...
          {results.length === 0 && !isGenerating ? (
            <div className="h-[600px] border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center text-gray-400 space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm">Tải ảnh lên và nhấn "Nâng cấp ảnh" để bắt đầu</p>
              </div>
            </div>
          ) : (
                ...
                {results.map((img) => (
                  ...
                  >
                    ...
                    <div className="p-4 bg-white/90 backdrop-blur-sm flex items-center justify-between">
                      <div>
                        <p className="text-xs font-mono text-gray-400 uppercase">{img.settings.style} • {img.settings.aspectRatio}</p>
      ...
      <footer className="max-w-7xl mx-auto px-6 mt-20 pt-8 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4 text-gray-400 text-xs">
        ...
        <div className="flex items-center gap-6">
          ...
          <a href="#" className="hover:text-brand-orange transition-colors">Hỗ trợ</a>
        </div>
      </footer>
    </div>
  );
